...
const getAverage = weightList => {
  const sum = weightList.reduce((acc,curr) => acc+curr,0) / weightList.length
  return Array(weightList.length).fill(sum)
}
...
watch(weights, newWeights => {
  ...
  if (weightChart.value) {
    ...
    weightChart.value.data.datasets[1].data = getAverage(ws
      .sort((a,b) => a.date - b.date)
      .map(w => w.weight))
      .slice(-30)
      ...
      data: {
        ...
        datasets: [
          ...
          },
          {
            label: 'Durchschnitt',
            type: "line",
            data: getAverage(ws.sort((a,b) => a.date - b.date).map(w => w.weight)),
            borderColor: 'blue',
            fill: false,
            order: 2
          }
